...
- [/] 示例语义已验证：仓 3 上料、仓 5 掉线、仓 2 恢复上线均触发对应 JSON 上报
- [/] 暂不处理 DTU 联网、ACK、重试、平台协议，仅串口输出 JSON → 后续阶段补充
- [ ] 删除废弃的 `BinWeightSyncPacket` 全仓仓重同步包（已删除 magic/struct/回调/广播函数）
- [ ] 主仓变化事件仍需硬件 485/BLE 恢复到现场才能产生真实数据流
