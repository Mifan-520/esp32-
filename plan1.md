...
DTU 上云策略已调整：任何仓发生上料、下料或编辑时，通过 ESP-NOW 广播事件及该来源仓操作后的本机仓重；DTU 节点再通过本机 GPIO32/GPIO25 串口上报 JSON。JSON 必须带事件来源仓号、变化重量和来源仓当前仓重。
...
- [x] JSON 字段（事件同时携带来源仓操作后的仓重，`seq` 由 sourceBin 独立递增）：
...
- **仓重**：每仓仓重保存在本地 NVS；上料、下料、编辑事件均携带操作后的 `binWeight`，云端以该值同步来源仓仓重并用 `deltaG` 统计上下料。
